...
def compute_orthogonality_deviation(model):
    ...
    for m in model.modules():
        if m.__class__.__name__.endswith("OrthogonalSkip") and hasattr(
            m, "_get_W_matrix"
        ):
            W = m._get_W_matrix()
            dev += torch.norm(
                W.T @ W - torch.eye(W.size(0), device=W.device), p="fro"
            ).item()

    # normalize by the number of orthogonal skips
    return dev / model.num_orthogonal_skips
